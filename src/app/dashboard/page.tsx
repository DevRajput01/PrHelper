    ...
    <div className="max-w-[1340px] mx-auto p-4 sm:p-6 lg:p-8">
      ...
      <div className="clay-card p-4 sm:p-7 grid grid-cols-1 lg:grid-cols-12 gap-7">
        ...
        <main className="lg:col-span-9 space-y-6 relative">
          ...
          {/* Live In-Place Result Card (Clean, Non-Deformed, Ultra-Structured) */}
          {activeItem && (
            <div className="clay-card p-6 sm:p-8 border-2 border-sky-400/90 shadow-2xl space-y-5 relative overflow-hidden">
              {/* Top Row: Badges & Action Buttons */}
              <div className="flex flex-wrap items-center justify-between gap-3 pb-3 border-b border-slate-100">
                <div className="flex items-center gap-2.5">
                  <span className={`px-3.5 py-1.5 rounded-full text-[11px] font-black uppercase tracking-wider text-white shadow-sm ${
                    ...
                    {activeItem.type === "reel" ? "📱 Instagram Reel Script" : activeItem.type === "short" ? "🎬 YouTube Short" : "🎨 Studio Visual Prompt"}
                  </span>
                  <span className="text-xs font-bold text-slate-500 bg-slate-100 px-3 py-1 rounded-full">
                    {activeItem.estimated_duration_seconds ? `⏱️ ${activeItem.estimated_duration_seconds}s` : "Ready-to-Use"}
                  </span>
                </div>
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="px-4 py-2 rounded-full clay-btn-white text-xs font-bold flex items-center gap-1.5 shadow-sm hover:text-sky-600 transition-all"
                  >
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-full text-xs font-black transition-all flex items-center gap-1.5 shadow-sm ${
                  ...
                  >
                    ...
                    <span>{activeItem.is_kept ? "Saved" : "Save to Library"}</span>
                  </button>
                </div>
              </div>
              ...
              {/* Main Content Script Box (Structured Rounded Card) */}
              <div className="clay-inset-card p-5 sm:p-6 bg-slate-50 border border-slate-200/90 text-slate-900 shadow-inner">
                <div className="flex items-center justify-between mb-3 pb-2 border-b border-slate-200/60">
                  <span className="text-[11px] uppercase font-black tracking-wider text-sky-700 flex items-center gap-1.5">
                    <Sparkles className="w-3.5 h-3.5" />
                    {activeItem.type === "image" ? "Diffusion Image Prompt" : "Production Script & Storyboard Breakdown"}
                  </span>
                  <button
                    onClick={() => handleCopy(activeItem.prompt_text)}
                ...
                <div className="text-xs sm:text-sm text-slate-800 leading-relaxed font-medium whitespace-pre-line space-y-2">
                  {activeItem.prompt_text}
                </div>
              </div>
              ...
              {activeItem.description && (
                <div className="p-4 rounded-2xl bg-sky-50/90 border border-sky-200/90 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-sm">
                  <div className="text-xs text-sky-950 font-medium">
                    <span className="font-black text-sky-900 block mb-0.5">📝 Caption & Copy:</span>
                    <p className="text-slate-800">{activeItem.description}</p>
                  </div>
                  <button
                    onClick={() => handleCopy(activeItem.description)}
                    className="shrink-0 px-3 py-1.5 rounded-full bg-white text-[11px] font-bold text-sky-700 border border-sky-300 shadow-sm hover:bg-sky-50 flex items-center gap-1 self-start sm:self-center"
                  >
                    <Copy className="w-3 h-3" />
                    Copy Caption
                  </button>
              ...
              {/* Rendered Image (if visual type) */}
              {activeItem.type === "image" && activeItem.assets && activeItem.assets.length > 0 && (
                <div className="pt-2">
                  <span className="text-[11px] uppercase font-black tracking-wider text-slate-600 block mb-2">
                    Rendered Studio Visual Preview
                  </span>
                  <div className="relative rounded-2xl overflow-hidden border-2 border-white shadow-lg group bg-slate-900">
                    <img
                    ...
                    <div className="absolute inset-0 bg-slate-900/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                      <a
                        ...
                        className="px-5 py-2.5 clay-btn-white text-slate-900 text-xs font-bold inline-flex items-center gap-1.5 shadow-lg"
                      >
